...
K_sq = [6 5 4 3 2 1 0];
moving_sq = [pi/18 pi/9 2*pi/9 4*pi/9 6*pi/9 8*pi/9];
...
for k = 1:20
    ...
    for i=1:equilibrate %equilibrate
        ...
        rotation_angle = (rand-0.5)*pi/9;%moving_sq(t);
        open_test = rand();
        open_node = 0;
        ...
        if open_test < open_p
            ...
            energy_old_open = K*(EED_end_test-r_0);
        end
        ...
        if open_test < open_p
            energy_trace_test_open = K*(EED_end_test-r_0);
            moving_test = moving_check(energy_old_open,energy_trace_test_open);
        end
